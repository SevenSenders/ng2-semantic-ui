...
<!-- Search input box -->
<input *ngIf="isSearchable" class="search" type="text" autocomplete="off" [(ngModel)]="query" (keydown)="searchKeyDown($event)" [class.not-empty]="selectedOptions.length">
<!-- Single-select label -->
<div *ngIf="!selectedOption" class="default text" [class.filtered]="query">{{ placeholder }}</div>
<!-- Select dropdown menu -->
<div class="menu" suiDropdownMenu>
    <ng-content></ng-content>
    <div *ngIf="!availableOptions.length && !maxSelectedReached" class="message">No Results</div>
    <div *ngIf="!availableOptions.length && maxSelectedReached" class="message">Max {{ maxSelected }} selections</div>
</div>
`,
    styles: [`
:host input.search {
...
:host input.search.not-empty {
    display: none;
}
:host.active input.search {
    display: inline;
}
.selected-results {
    display: none;
...
export class SuiMultiSelect implements AfterContentInit, AfterViewInit {
    ...
    public set options(value: any[]) {
        if (this.isRemote) {
            this._searchService.options = [...value, ...this.selectedOptions].reduce((opts, val) => {
                const valStr = JSON.stringify(val);

                for (let o of opts) {
                    if (JSON.stringify(o) == valStr) {
                        return opts;
                    }
                }
        ...
        if (this.options.length > 0 && this.selectedOptions) {
            this.selectedOptions = this.selectedOptions.map(so => {
                if (!this.options.find(o => JSON.stringify(o) == JSON.stringify(so))) {
    ...
    @Output()
    public onQueryChanged: EventEmitter<string> = new EventEmitter<string>();
    ...
    private get availableOptions(): Array<any> {
        if (this.isRemote) {
            return this._searchService.options.filter(o => {
                const os = JSON.stringify(o);

                if (o[this.displayField].toLowerCase().indexOf(this.query.toLowerCase()) != -1) {
                    for (let so of this.selectedOptions) {
                        if (os == JSON.stringify(so)) {
                            return false;
                        }
                    }
                } else {
